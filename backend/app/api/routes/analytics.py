...
@router.get("/overview")
def get_analytics_overview(db: Session = Depends(get_db), admin = Depends(get_current_admin)):
    """Yields strict structural KPIs for the smart dashboard."""
    return analytics_svc.get_overview(db)

@router.get("/trends")
def get_inspection_trends(days: int = 7, db: Session = Depends(get_db), admin = Depends(get_current_admin)):
    """Time-series execution block."""
    return analytics_svc.get_trends(db, days)

@router.get("/ai-observations")
def get_ai_observations(db: Session = Depends(get_db), admin = Depends(get_current_admin)):
